<?php
/**
 * Controleur de l'administration des joueurs pour débannir
 *
    ...
    if (array_key_exists($id, $players)) {
        if ($players[$id]['rank'] <= 2) {
            if (!empty($players[$id]['ip'])) {
                ...
                $retvalue = System::unbanPlayer($players[$id]['ip']);
                ...
                if ($retvalue != 0) {
                    ...
                    $app['session']->getFlashBag()->add('success', 'Le joueur ' . $players[$id]['login'] . ' est maintenant débanni du réseau');
                }
            } else {
                $app['session']->getFlashBag()->add('error', 'Impossible de débannir ce joueur car il n\'a pas d\'adresse IP');
            }
        }
    }
